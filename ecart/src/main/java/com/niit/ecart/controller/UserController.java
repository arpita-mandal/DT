...
public class UserController {
	  ...
	  @RequestMapping("/403")  
	    public ModelAndView accessisdeined(){  
	 ...
	 public ModelAndView save(@ModelAttribute("users") User users){ 
		 String uid=users.getUserId();
		  ...
		  User temp=usersDao.getUsersById(uid);
		  
		  if(temp!=null)
		  {
			  ModelAndView  mv= new ModelAndView("usersform","command",users); 
		 		//mv.addObject("customerList", usersDao.showAllUser());
		 		mv.addObject("error", "UserId already exists");
		         return mv;
		  }
